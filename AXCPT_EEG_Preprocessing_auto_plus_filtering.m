...
for kk=1:Num_folders
    ...
    % Define the paths ; subject and session 
...
%         disp('This prints when you finished closing the window');
       % EEG = pop_select( EEG,'nochannel',{'EXG5' 'EXG6' 'EXG7' 'EXG8' 'EXG3' 'EXG4' 'GSR1' 'GSR2' 'Erg1' 'Erg2' 'Resp' 'Plet' 'Temp'});
        ...
        from=0;
        to=20;
        lowpass_filt=eegfiltfft(data, fs_new, from, to);
...
%         EEG.data=data_filt_smooth;
        % Epoch
        NameForSet='_ch_DC_epochs_tr2_auto_3_chan_filt';
        ...
        EEG = pop_epoch( EEG, {  trigger }, [from_sec to_sec], 'newname', temp_epochname, 'epochinfo', 'yes');
        EEG = eeg_checkset( EEG );
        baseline_from=-500;
        baseline_to=-450;
